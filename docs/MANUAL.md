...
- **Solo planes fijos, no recarga libre** (todos los agentes + Tool_Procesador_Pagos, deploy 2026-07-06): bot NO ofrece recargar montos arbitrarios al wallet. Solo montos exactos de planes (PE S/160/S/298/S/390, CO $298K/$390K, MX $1,910/$2,499). Rechaza "reponer" consumo foodcourt o Yape libre.
- **Dirección VERBATIM** (ATC + Ventas, deploy 2026-07-06): al confirmar cobertura o repetir dirección del cliente, bot copia EXACTO lo escrito. Prohibido reformatear ("Calle"→"Cl."), cambiar números/letras, alterar barrio o rellenar campos alucinados. Si sospecha typo → preguntar, no alterar.
- **Consulta pedido: preguntar tipo primero** (ATC FASE 4, deploy 2026-07-03 3er): PE — antes de dar detalles del pedido (estado/hora/contenido), bot pregunta "¿Menú Diario o Restaurante?" y espera. Excepciones: solo un pedido activo en Info ATC, cliente ya especificó tipo, o adjuntó captura del pedido. CO/MX asumen Menú Diario.
- **Cifras de costo — nunca inventar** (ATC banner top, deploy 2026-07-03 2do): aplica a cualquier costo (cambio dirección, delivery, retorno, cargos variables). Si Info ATC no te da el número, bot dice "el valor lo verás en la app / lo confirma el asesor". PROHIBIDO cotizar S/6, $8000, cualquier cifra específica inventada.
- **Cambio dirección same-day: no cotizar valor** (ATC, deploy 2026-07-03): bot NUNCA inventa cifra específica (S/6, $8000). Monto lo calcula la app según ruta o lo confirma asesor. Bot dice "el valor lo verás en la app / lo confirma el asesor".
- **CASO E retorno con status CLIENTE NO CONTESTÓ** (ATC, deploy 2026-07-02): trigger duro — si `delivery_status = CLIENTE NO CONTESTÓ` + cliente pregunta cómo recibir → CASO E siempre (4 pasos app, valor extra, cutoff 1:30 pm). NO aplicar REGLA REPARTIDOR CONTACTA (pre-entrega). Prohibido "espera 5 min / coordinar con repartidor" cuando ya se fue.
- **Nombre cliente + pedido no encontrado** (ATC, deploy 2026-07-02): prohibido "estimado cliente". Usar `WHATSAPP.name` (ManyChat) si Info ATC no tiene nombre. Si cliente menciona pedido específico + backend no lo encuentra → pedir email o número con el que hizo el pedido, no responder "no logré entenderte".
- **Amenaza escalación legal / otras instancias** (ATC, deploy 2026-07-01): cuando cliente menciona INDECOPI/PROFECO/SIC/abogado/demanda/otras instancias/redes sociales → de-escalar con empatía + derivar URGENTE a asesor senior. NUNCA despedir casual ni ecoar la amenaza. NO admitir responsabilidad ni negociar por WhatsApp.
- **Cambio número de teléfono NO es auto-gestión** (ATC + Ventas + Reconsumos, deploy 2026-07-01): no se hace desde el app. Bot pide ambos números (actual + nuevo) en turno 1, en turno 2 confirma "[actual] → [nuevo]" y deriva a asesor. Prohibido enviar deeplink `n8n-auth-` o decir "actualízalo desde la app".
- **Eliminar cuenta / dar de baja sin derivar** (ATC, deploy 2026-07-01): trámite estandarizado de 24-48h hábiles. Bot responde con plantilla "registramos tu solicitud... 24 a 48 horas hábiles" SIN derivar. Equipo administrativo procesa en backend.
- **Precio Cena 600 kcal (Aumentar masa)** (Ventas + Calculadora, deploy 2026-06-30): vale lo mismo que Almuerzo 600 kcal — PE S/21.50 → S/15.50 con plan, CO $21,500 → $15,500. NUNCA usar precios de Cena 400 kcal (S/17.50/S/11.50 o $17,500/$11,500). Calculadora incluye explícitamente "Cena 600 kcal (Aumentar masa)" en lista PE y CO.
- **Facturas por país** (ATC, deploy 2026-06-23): PE pide RUC/DNI + razón social + cocina. **MX pide Constancia de Situación Fiscal (PDF) + RFC + régimen fiscal + código postal + dirección fiscal + uso CFDI + forma de pago**. CO pide NIT/cédula + correo + dirección. PROHIBIDO en MX pedir "RUC/DNI" — usar RFC + PDF SAT.
- **GATE PLAN ACTIVO estricto** (Ventas, deploy 2026-06-22): para saludar como cliente existente / mencionar plan activo, OBLIGATORIO 4 condiciones AND: `last_monthly_plan != null` + `amount > 0` + `end_date >= hoy` + `status_name` literalmente activo/vigente/recurrente. Si cualquiera falla → tratar como VENTAS_NUEVO. status_id solo no es suficiente (puede ser "registro"/"lead").
- **Alcance del asesor en cambio dirección** (ATC, deploy 2026-06-15): asesor SÍ cambia dirección SOLO para Restaurante. Para Menú Diario el cambio se hace ÚNICAMENTE desde la app del cliente — ni el bot ni el asesor pueden gestionarlo. Si Menú Diario y app bloquea (cutoff pasado), ofrecer alternativas CASO C A.2 (cambio dirección con valor adicional o donación). Banner STOP refuerza PASO 1.5 (preguntar tipo antes de ejecutar Deeplink o dar pasos).
...
- Bot ofrece "te conecto con asesor para gestionar el cambio de dirección" cuando el pedido es Menú Diario → regresión de la política ALCANCE DEL ASESOR. Asesor SOLO cambia direcciones de Restaurante.
- Bot da pasos del app sin preguntar primero Menú Diario vs Restaurante → regresión del PASO 1.5 (con el banner STOP del 15-jun debe ser imposible omitirlo).
- Bot dice "¡Qué bueno saludarte de nuevo! Ya tienes plan activo" a lead nuevo que nunca pagó → regresión del GATE PLAN ACTIVO. Verificar 4 condiciones AND (last_monthly_plan, amount>0, end_date>=hoy, status_name literal activo/vigente).
- Bot pide "RUC/DNI" para factura a cliente MX → regresión. MX usa RFC + Constancia de Situación Fiscal (PDF SAT) + régimen fiscal + código postal.
- Bot dice "cena 600 kcal S/17.50 → S/11.50" (PE) o "$17,500 → $11,500" (CO) para Aumentar masa → mezcla con Cena 400. Correcto: Cena 600 kcal = mismo precio que Almuerzo 600 (S/21.50 → S/15.50 / $21,500 → $15,500).
- Bot deriva a asesor para "eliminar cuenta / dar de baja" → regresión. Atender directo con plantilla 24-48h.
- Bot dice "actualiza tu número desde tu perfil en la app" → regresión. Pedir ambos números y derivar a asesor.
- Bot despide casual ("que te vaya bien", "nos vemos en otras instancias") ante cliente que amenaza escalar a INDECOPI/abogado/demanda → regresión crítica. Debe empatizar + derivar URGENTE con flag.
- Bot dice "estimado cliente" cuando ManyChat tiene name → regresión NOMBRE. Debe usar WHATSAPP.name como fallback.
- Cliente menciona pedido específico ("ayer hice pedido para hoy") + backend no lo encuentra + bot responde "no logré entenderte" → regresión. Debe pedir correo o número de registro.
- Bot dice "el repartidor espera 5 min / coordinar por WhatsApp" cuando `delivery_status = CLIENTE NO CONTESTÓ` → regresión CASO E. Debe aplicar retorno (4 pasos app), no guion pre-entrega.
- Bot cotiza cifra específica de cambio dirección same-day ("aparece S/6") → fabricación. Debe decir "el valor lo verás en la app o lo confirma asesor".
- Bot da detalles de pedido sin preguntar Menú Diario o Restaurante en PE (con posible ambigüedad) → regresión FASE 4 REGLA #0. Preguntar primero, esperar respuesta.
- Bot confirma dirección distinta a la escrita por cliente (ej. "Calle 10"→"Cl. 4", cambia barrio) → regresión DIRECCION VERBATIM. Copiar exacto.
- Bot ofrece "recargar ese monto a tu wallet" con cantidad libre (Yape por S/50, reponer consumo foodcourt) → regresión MONTOS DE RECARGA. Solo planes fijos.
